...
export default function GameScoreModal({ open, onClose, selectedGame, users, onAssignPoints, onDeleteGame }) {
  return (
      ...
      <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 400, bgcolor: "#1e1e1e", color: "white", boxShadow: 24, p: 4, borderRadius: 2 }}>
        {selectedGame && (
          <>
            ...
            {users.map(user => (
              <Button key={user.id} variant="contained" color="primary" fullWidth sx={{ mt: 1 }} onClick={() => onAssignPoints(user.id)}>
                {user.name} +10 Puntos
              </Button>
            ))}
            <Button variant="contained" color="secondary" fullWidth sx={{ mt: 1 }} onClick={() => onDeleteGame(selectedGame)}>
              Quitar juego
            </Button>
          </>
        )}
